...
* value is returned.
*/
static ssize_t servo_read(struct file *file, char __user *buf, size_t count, loff_t *offset){
    size_t ret;
    u32 val;
/*
...
* value is returned.
*/
static ssize_t servo_write(struct file *file, const char __user *buf, size_t count, loff_t *offset){
    size_t ret;
    u32 val;
    struct servo_dev *priv = container_of(file->private_data,
    struct servo_dev, miscdev);
